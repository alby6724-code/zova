...
export interface AdCreditPackage {
  id: string;              // Stable slug, e.g. 'pkg-platinum'
  tierId?: string;
  name: string;            // Display name, e.g. 'Platinum'
  description: string;     // Short description for marketplace display
  credits: number;         // Number of featured ad slots included
  adsCount?: number;
  durationDays: number;    // Days each boosted ad stays featured
  durationValue?: number;
  durationUnit?: 'Month' | 'Days' | 'Year';
  price: number;           // Price in ₹ (paise-free, whole number)
  currency?: string;
  badge?: string;          // Optional label, e.g. 'Most Popular', 'Best Value'
