...
function getInitialAdminPasswordHash(): string {
  ...
  // Pre-computed bcrypt hash (rounds=12) of the default admin password.
  // This is NOT the plaintext password — it is a one-way hash.
  // To override, set INITIAL_ADMIN_SECRET or INITIAL_ADMIN_PASSWORD_HASH in Vercel env vars.
  return '$2a$12$v.VF9XSKSCoJ.aOuN7X2Eevh9XLn4oM2cOPBoGUdjRHGpLufm1RnC';
}

// Resolve admin email and name from environment variables (set these in Vercel).
// Default is the real production admin email so login works without any env vars.
...
class DatabaseStore {
  ...
  private seed() {
    // 1. Administrator Account — credentials driven by env vars (set in Vercel):
    //    ADMIN_EMAIL, ADMIN_NAME, INITIAL_ADMIN_SECRET or INITIAL_ADMIN_PASSWORD_HASH
  ...
  // --- PREMIUM AD CREDIT PACKAGES CRUD ---
